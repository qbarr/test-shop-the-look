...
class ProductFormComponent extends Component {
  ...
  #processBatchAddToCart(items) {
    ...
    return fetch(Theme.routes.cart_add_url, {
      ...
      .then(async (response) => {
        ...
        if (this.refs.quantitySelector || this.refs.quantityLabelCartCount) {
          await this.#fetchAndUpdateCartQuantity();
        }
  ...
  /**
   * Public batch-add entry point used by the look page.
   * Reuses the theme's existing cart add flow and events.
   * @param {Array<{variantId: string, quantity: number}>} items
